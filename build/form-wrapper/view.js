...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateFormValidation(form, submitButton) {
    ...
    if (isFormValid) {
      ...
      submitButton.setAttribute('title', 'Please fill out all required fields correctly');
    }
  }
  ...
  function handleFormSubmission(form, submitButton) {
    ...
    const successMessage = getFormAttribute(formWrapper, 'success-message');
    const errorMessage = getFormAttribute(formWrapper, 'error-message');

    // Submit form
    submitForm(formData, form.id || 'form-wrapper', successMessage, errorMessage).then(function (response) {
      if (response.success) {
        showSuccessMessage(form, response.message || 'Form submitted successfully!');
        form.reset();
        ...
        if (response.errors && typeof response.errors === 'object') {
          ...
          showErrorMessage(form, response.message || 'Form submission failed. Please try again.');
        }
  ...
  function collectFormData(form) {
    const data = {};
    const formElements = form.querySelectorAll('input[type="text"], input[type="email"], input[type="password"], input[type="url"], input[type="tel"], textarea');
  ...
  function validateInputWithMessages(input) {
    ...
    const customInvalidMessage = input.getAttribute('data-invalid-message');

    // Check required
    if (input.required && !value) {
      showInputError(input, customInvalidMessage || getErrorMessage('required'));
      return false;
    }
    ...
    const minLength = input.getAttribute('minLength');
    if (minLength && value.length < parseInt(minLength)) {
      showInputError(input, customInvalidMessage || getErrorMessage('minLength', minLength));
      return false;
    }
    ...
    const maxLength = input.getAttribute('maxLength');
    if (maxLength && value.length > parseInt(maxLength)) {
      showInputError(input, customInvalidMessage || getErrorMessage('maxLength', maxLength));
      return false;
    }
    ...
    if (pattern && value) {
      const regex = new RegExp(pattern);
      if (!regex.test(value)) {
        showInputError(input, customInvalidMessage || getErrorMessage('pattern'));
        return false;
      }
    }
    ...
    switch (input.type) {
      case 'email':
        if (!isValidEmail(value)) {
          showInputError(input, customInvalidMessage || getErrorMessage('email'));
          return false;
        }
        break;
      case 'url':
        if (!isValidUrl(value)) {
          showInputError(input, customInvalidMessage || getErrorMessage('url'));
          return false;
        }
        break;
      case 'tel':
        if (!isValidTel(value)) {
          showInputError(input, customInvalidMessage || getErrorMessage('tel'));
          return false;
        }
        break;
    }
  ...
  function submitForm(formData, formId, successMessage, errorMessage) {
    const apiUrl = window.formBuilderAjax ? window.formBuilderAjax.apiUrl : (window.wpApiSettings?.root || '/wp-json/') + 'form-builder/v1/submit';
    const nonce = window.formBuilderAjax ? window.formBuilderAjax.nonce : '';
  ...
  function showServerValidationErrors(form, errors) {
    Object.keys(errors).forEach(function (fieldId) {
      const input = form.querySelector(`#${fieldId}, [name="${fieldId}"]`);
      if (input) {
        const errorMessage = Array.isArray(errors[fieldId]) ? errors[fieldId][0] : errors[fieldId];
  ...
  function showFieldError(input, message) {
    ...
    if (input.classList.contains('text-input-field')) {
      ...
      const existingErrors = input.parentNode.parentNode.querySelectorAll('.text-input-error');
      ...
      input.classList.remove('border-gray-200', 'focus:border-blue-500', 'focus:ring-blue-500', 'border-teal-500');
  ...
  function getErrorMessage(type, value) {
    ...
    switch (type) {
      ...
      case 'minLength':
        return (messages.minLength || 'Please enter at least %d characters.').replace('%d', value);
      case 'maxLength':
        return (messages.maxLength || 'Please enter no more than %d characters.').replace('%d', value);
      case 'pattern':
        return messages.pattern || 'Please enter a valid value.';
      case 'email':
        return messages.email || 'Please enter a valid email address (e.g., user@example.com).';
      case 'url':
        return messages.url || 'Please enter a valid URL (e.g., https://example.com).';
      case 'tel':
        return messages.tel || 'Please enter a valid phone number (e.g., +1-234-567-8900).';
      default:
  ...
  function showInputSuccess(input, message) {
    ...
    input.classList.remove('border-gray-200', 'focus:border-blue-500', 'focus:ring-blue-500', 'border-red-500');
  ...
  function resetInputField(input) {
    ...
    input.classList.remove('border-red-500', 'focus:border-red-500', 'focus:ring-red-500', 'border-teal-500', 'focus:border-teal-500', 'focus:ring-teal-500');
    input.classList.add('border-gray-200', 'focus:border-blue-500', 'focus:ring-blue-500');
  ...
  function clearInputErrors(input) {
    // Remove error messages
    const errorMessages = input.parentNode.parentNode.querySelectorAll('.text-input-error');
    errorMessages.forEach(function (error) {
      error.remove();
    });

    // Remove success messages
    const successMessages = input.parentNode.parentNode.querySelectorAll('.text-input-success');
